...
public class CustomExceptionHandler extends ResponseEntityExceptionHandler {
	...
	protected ResponseEntity<Object> handleHttpMessageNotReadable(
			HttpMessageNotReadableException ex, org.springframework.http.HttpHeaders headers, HttpStatus status, WebRequest request) {

		if(ex.getMessage().toLowerCase().contains(ResponseMessage.MSG_ERR_MISSING_REQUEST_BODY.toLowerCase())){
			return createResponseEntity(new ResponseObject(ResponseMessage.MSG_ERR_MISSING_REQUEST_BODY), HttpStatus.BAD_REQUEST);
		}
		
		else if(ex.getMessage().toLowerCase().contains("json parse error: unexpected character")) {
			return createResponseEntity(new ResponseObject(ResponseMessage.MSG_ERR_REQUEST_INVALID), HttpStatus.BAD_REQUEST);
		}
